...
def subprocess_adb(cmd, device_id):
    if device_id:
...
def call_adb(cmd, device_id, timeout=0):
    signal.signal(signal.SIGALRM, sig_unresponsive_handler)
    signal.alarm(timeout)
    p = subprocess_adb(cmd, device_id)
    try:
        ...
        p.terminate()
        signal.alarm(0)
        raise DeviceUnresponsiveException(
            "Device {} does not respond.".format(device_id))
    signal.alarm(0)
    return out, err


def cat_file(file_, device_id, has_eof=True):
    """ Cat a file on the device """
    if has_eof:
        out, err = execute_privileged_command("cat {}".format(file_),
                                              device_id)
    else:
...
def set_date(device_id):
    ...
    out, err = execute_privileged_command(
        "TZ=CEST date {} && TZ=CEST date +%Y%m%d%H%M".format(formatted_time),
        device_id, 5)
    ...
    # there were situations where this code results in +/-1 second
    # we still consider local and remote to be in sync for +/-1 sec situations
    local_time = int(time.strftime('%Y%m%d%H%M', current_time).encode())
    remote_time = int(out_lines[1].strip())
    if abs(remote_time - local_time) > 1:
...
def execute_privileged_command(cmd, device_id, timeout=0):
    """ Executes the given privileged command on the device """

    out, err = call_adb(["shell", "su -c '{}'".format(cmd)], device_id,
                        timeout)
    if err or b"invalid uid" in out:
        out, err = call_adb(["shell", "su root sh -c '{}'".format(cmd)],
                            device_id, timeout)
    if err:
...
def execute_command(cmd, device_id, timeout=0):
...
def remove_forward(device_id):
    """ Remove socket forwarding """
    out, err = call_adb(["forward", "--remove"], device_id)
    return out


def forward(lport, dport, device_id):
    """ Forward socket connection """
    out, err = call_adb(
        ["forward", "tcp:{}".format(lport), "tcp:{}".format(dport)], device_id)
    return out
...
def pull_privileged(what, where, device_id):
    """ Pull from the device """
    ...
    execute_privileged_command(
        " ".join(["chown", "shell:shell", workdir_file]), device_id)
...
def wait_device_booted(device_id):
    ...
    while True:
        try:
            out, _ = execute_command("getprop init.svc.bootanim",
                                     device_id,
                                     timeout=5)
            if not out.strip():
...
def is_recovery(device_id):
    """ Check if we are in recovery mode. """
    out, _ = execute_command("getprop init.svc.recovery", device_id)
    if b"running" in out:
        return True
    return False


def program_exists(program_name, device_id):
...
def path_exists(path, device_id):
...
def pgrep(process_name, device_id):
    """ Grep process by name """
    out = b""
    if program_exists("pgrep", device_id):
        tmp_out, _ = execute_privileged_command(
            "pgrep {}".format(process_name), device_id)
        out += tmp_out
    else:
...
def pidof(process_name, device_id):
...
def pkill(process_name, device_id):
